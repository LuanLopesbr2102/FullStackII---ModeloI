...
export default function BarraBusca({ placeHolder,
    ...
    const [vagaSelecionado, setSelecionado] = useState(false);

    function filtrarResultado() {
        //exige que o termo da busca seja conhecido e que
        //esse termo seja utilizado como critério de seleção
        setDadosLista(dados.filter((vaga) => {
            return termoBusca.length > 1 ? vaga[campoBusca].toLowerCase().includes(termoBusca.toLowerCase()) : false
        }
        )
        );
                ...
                <ul data-resultado>
                    {
                        dadosLista.map(vaga => {
                            ...
                            return <li key={vaga[campoChave]}
                                onClick={() => {
                                    setTermoBusca(vaga[campoBusca]);
                                    setSelecionado(true);
                                    funcaoSelecao(vaga);
                                ...
                                {
                                    vaga[campoChave] + '-' + vaga[campoBusca]
                                }
                            </li>
                        })
                    }
