...
export default class ClienteCTRL{
    ...
    gravar(requisicao, resposta){
        ...
        if(requisicao.method === "POST" && requisicao.is('application/json')){
            ...
            const uf = dados.uf;
            if(cnpj && nome && telefone && email && endereco && bairro && cidade && uf)
            {
                //gravar esse cliente
                const cliente = new Cliente(0,cnpj, nome, telefone, email, endereco, bairro, cidade,
                                            uf);
    ...
    atualizar(requisicao, resposta){
        ...
        if(requisicao.method === "PUT" && requisicao.is('application/json')){
            ...
            {
                //gravar esse cliente
                const cliente = new Cliente(cnpj, nome, telefone,email, endereco, bairro, cidade,
                                            uf);
                //método assíncrono gravar que instancia a camada de persistência e
                //grava um cliente no banco de dados
    ...
    excluir(requisicao, resposta){
        ...
        if(requisicao.method === "DELETE" && requisicao.is('application/json')){
            ...
            {
                //gravar esse cliente
                const cliente = new Cliente(cnpj);
    ...
    consultarPeloCPF(requisicao, resposta){
        ...
        if(requisicao.method === "GET"){
            const cliente = new Cliente();
            //método assíncrono que recupera os clientes do banco dados
            cliente.consultarCPF(cnpj).then((cliente)=>{
