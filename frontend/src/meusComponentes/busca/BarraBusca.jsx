...
export default function BarraBusca({ placeHolder,
    ...
    const [funcSelecionado, setFincSelecionado] = useState(false);

    function filtrarResultado() {
        //exige que o termo da busca seja conhecido e que
        //esse termo seja utilizado como critério de seleção
        setDadosLista(dados.filter((func) => {
            return termoBusca.length > 1 ? func[campoBusca].toLowerCase().includes(termoBusca.toLowerCase()) : false
        }
        )
        );
    ...
    return (
        <Container>
            <div className='barra text-bg-light p-3'>
            ...
            <div className='resultado text-bg-light p-3'>
                <ul data-resultado>
                    {
                        dadosLista.map(func => {
                            //ex: cliente campoChave=cpf campoBusca=nome
                            return <li key={func[campoChave]}
                                onClick={() => {
                                    setTermoBusca(func[campoBusca]);
                                    setFincSelecionado(true);
                                    funcaoSelecao(func);
                                ...
                                {
                                    func[campoChave] + '-' + func[campoBusca]
                                }
                            </li>
                        })
                    }
